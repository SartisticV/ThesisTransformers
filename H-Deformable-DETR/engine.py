...
def evaluate(
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        for i in range(100):
            ...
            x1, y1, x2, y2 = torch.round(results[0]['boxes'][i]).tolist()
            bb.append({'image_id': targets[0]['image_id'].item(),
                       'category_id': results[0]['labels'][i].item(),
    ...
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    utils.save_on_master_json(bb, open(os.path.join(output_dir, f'prediction{epoch}.json'), 'w'))
    #if epoch is not None:
    #    writer.add_scalar('validation loss', metric_logger.loss.global_avg, epoch)
    print("Averaged stats:", metric_logger)
    if coco_evaluator is not None:
