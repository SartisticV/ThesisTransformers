...
import util.stacktracer
import datasets.samplers as samplers
from datasets import build_dataset, get_coco_api_from_dataset
from engine import evaluate, train_one_epoch
from models import build_model
from util import stacktracer
...
def get_args_parser():
    ...
    parser.add_argument("--dlr", default=1.0, type=float)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="sf",
        type=str,
        choices=("sf", "enc", "sum", "mha", "conv"),
        help="Type of fusion mechanic to reduce temporal dimension in the backbone output. Currently available: enc, sum, mha",
    )
    ...
    parser.add_argument("--coco_path", default="/scratch/6409458/data", type=str)
    ...
    parser.add_argument('--pretrained2d', action="store_true")
...
def main(args):
    ...
    dataset_train = build_dataset(image_set="visdrone", args=args, test=False)
    dataset_val = build_dataset(image_set="vistest", args=args, test = True)
    ...
    if args.resume:
        if args.resume.startswith("https"):
            ...
            print('checkpoint', checkpoint['model']['backbone.0.body.layers.0.blocks.0.attn.qkv.weight'])
            if 'video' in args.backbone and not args.eval:
                l = [k for k in checkpoint['model'].keys() if "backbone" in k]
                for k in l:
            ...
            if not args.eval:
                ...
                for k in list(checkpoint['model'].keys()):
                    if 'class_embed' in k:
                        print('removing', k)
                        del checkpoint['model'][k]
                # Adjust weights for new encoder input shape
                if args.fusion == 'enc':
                    for i in range(args.num_feature_levels):
                        checkpoint['model'][f'input_proj.{i}.0.weight'] = torch.unsqueeze(checkpoint['model'][f'input_proj.{i}.0.weight'], dim=2).expand(model_without_ddp.state_dict()[f'input_proj.{i}.0.weight'].shape)
                        checkpoint['model'][f'input_proj.{i}.2.weight'] = checkpoint['model'][f'input_proj.{i}.1.weight']
                        checkpoint['model'][f'input_proj.{i}.2.bias'] = checkpoint['model'][f'input_proj.{i}.1.bias']
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            checkpoint_paths.append(output_dir / f"checkpoint{epoch:04}_{args.fusion}.pth")
