...
class TransformerBackbone(nn.Module):
    def __init__(
        ...
        if args.fusion == 'enc':
            ...
            from .video_swin_transformer_sum import SwinTransformer3D
        elif args.fusion == 'mha':
            from .video_swin_transformer_mha import SwinTransformer3D
        elif args.fusion == 'conv':
            from .video_swin_transformer_conv import SwinTransformer3D
        elif args.fusion != 'sf':
            raise NotImplementedError(f"Fusion type {args.fusion} not implemented!")

        print(f'fusion: {args.fusion}')

        if backbone == "video_swin_tiny":
            backbone = SwinTransformer3D(embed_dim=96,
                        patch_size=(2,4,4),
                        depths=[2, 2, 6, 2],
                        num_heads=[3, 6, 12, 24],
                        window_size=(8, 7, 7),
        ...
        elif backbone == "video_swin_base":
            backbone = SwinTransformer3D(embed_dim=128,
                        patch_size=(4,4,4),
                        depths=[2, 2, 18, 2],
                        num_heads=[4, 8, 16, 32],
                        window_size=(8, 7, 7),
            ...
            embed_dim = 128
            backbone.init_weights(args.pretrained_backbone_path)
        elif backbone == "video_swin_large":
            backbone = SwinTransformer3D(embed_dim=192,
                        patch_size=(4,4,4),
                        depths=[2, 2, 18, 2],
